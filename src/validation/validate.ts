...
export function validateAsset(
  ...
  for (const input of getContextInputs(asset)) {
    ...
    if (input.regex && input.allow_regex) {
      warnings.push({
        code: 'POK015',
        message: `Context input "${input.name}" sets both regex and allow_regex; allow_regex takes precedence.`,
        filePath,
      });
    }

    const checks: Array<{ pattern: string; kind: 'allow_regex' | 'deny_regex' | 'regex' }> = [];
    ...
    if (input.regex) checks.push({ pattern: input.regex, kind: 'regex' });
